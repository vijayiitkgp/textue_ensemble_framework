...
from models.ENSEMBLE18 import Net as ENSEMBLE18
...
def initialize_model(opt):  
    if opt.model == 'ENSEMBLE':
        model = eval('ENSEMBLE{}'.format(opt.backbone[-2:]))(opt)
...
def get_Scheduler(opt, optimizer):
    ...
    if opt.scheduler == 'onecyclelr':
        ...
        scheduler1 = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=10, verbose=False, threshold=0.0001, threshold_mode='rel', cooldown=0, min_lr=0, eps=1e-08)
        scheduler2 = lr_scheduler.StepLR(optimizer, step_size=1, gamma=1)
