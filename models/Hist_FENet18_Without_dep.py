...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("x_temp size is", x_temp.shape)
        x_hist = torch.flatten(self.histogram_layer(x_temp), start_dim=1)
        # print("x_hist shape is:", x_hist.shape)
        ...
        #
        # x0 = x0.unsqueeze(1).expand(x0.size(0), x3.size(1), x0.size(-1))
        # x = x0 * x3.unsqueeze(-1)
        # x = x.view(-1, x0.size(-1) * x3.size(1))
        x = self.fc(x)

        # FROM DEEP ENCODING POOLING
        # x1 = self.head(x)
        # x2 = self.pool(x)
