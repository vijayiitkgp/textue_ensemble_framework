...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x3 = torch.cat((x0,x2),1) #concatinate Fenet and Depnet
        x1 = x1.unsqueeze(1).expand(x1.size(0),x3.size(1),x1.size(-1))  #Bilinear Models
        x4 = x1*x3.unsqueeze(-1) #Bilinear Models
        x4 = x4.view(-1,x1.size(-1)*x3.size(1)) #Bilinear Models
